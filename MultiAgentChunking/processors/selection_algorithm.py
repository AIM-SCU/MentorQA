...
class SelectionAlgorithm:
    ...
    def compute_quotas(self, segments, K):
        ...
        if remainder > 0:
            order = sorted(
                segments.keys(), key=lambda s: raw[s] - math.floor(raw[s]), reverse=True
            )
            for s in order:
                if remainder == 0:
                    break
    ...
    def select_proportionally_distributed(self, items, K, skip_topics=None):
        ...
        filtered_items = [
            x for x in items if x.get("segment_topic", "").lower() not in skip_topics
        ]
        ...
        # checking the score and the quotas
