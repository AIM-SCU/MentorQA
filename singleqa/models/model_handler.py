...
class QAModel:
    ...
    def generate_qa_pairs(self, transcript):
        ...
        # if not isinstance(transcript, (str, list)):
        # # Try to extract text fields from dict, else stringify
        #     transcript = transcript.get("text") or transcript.get("transcript") or str(transcript)

        # if isinstance(transcript, list) and transcript and not isinstance(transcript[0], str):
        #     transcript = [d.get("text") or d.get("transcript") or str(d) for d in transcript]


        if isinstance(transcript, list):
        # Handle list of dicts like transcript.json
            if transcript and isinstance(transcript[0], dict):
                transcript = [(d.get("text") or d.get("transcript") or "").strip() for d in transcript]
            # Join into single string context
            transcript = " ".join([t for t in transcript if t])

        elif isinstance(transcript, dict):
            transcript = (transcript.get("text") or transcript.get("transcript") or "").strip()
